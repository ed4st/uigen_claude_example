...
test("MessageList renders user messages", () => {
  render(<MessageList messages={[msg("1", "user", "Create a button component")]} />);
  expect(screen.getByText("Create a button component")).toBeDefined();
});

test("MessageList renders assistant messages", () => {
  render(
    <MessageList messages={[msg("1", "assistant", "I'll help you create a button component.")]} />
  );
...
test("MessageList shows content for assistant message with content", () => {
  render(
    <MessageList messages={[msg("1", "assistant", "Generating your component...")]} isLoading={true} />
  );
...
test("MessageList doesn't show loading state for non-last messages", () => {
  const messages: UIMessage[] = [
    msg("1", "assistant", "First response"),
    msg("2", "user", "Another request"),
  ];
  render(<MessageList messages={messages} isLoading={true} />);
  expect(screen.queryByText("Generating...")).toBeNull();
});
...
test("MessageList renders multiple messages in correct order", () => {
  const messages: UIMessage[] = [
    msg("1", "user", "First user message"),
    msg("2", "assistant", "First assistant response"),
    msg("3", "user", "Second user message"),
    msg("4", "assistant", "Second assistant response"),
  ];
...
test("MessageList applies correct styling for user vs assistant messages", () => {
  const messages: UIMessage[] = [
    ...
    msg("2", "assistant", "Assistant message"),
  ];
...
test("MessageList handles empty content with parts", () => {
  const messages: UIMessage[] = [
    { id: "1", role: "assistant", parts: [{ type: "text", text: "This is from parts" }] },
  ];
